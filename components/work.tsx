			...
			<div className='grid md:grid-cols-2 gap-4'>
				{WORK.map((w) => (
					...
					>
						...
						<div className='absolute top-0 w-full h-full group-hover:bg-black/70 z-[1] transition-all duration-1000' />
						{/* <div className='absolute bottom-0 w-full h-1/5 group-hover:bg-white/10 z-[1] transition-all duration-1000' /> */}

						<Link
							href={w.link}
							target='_blank'
							className='hidden group-hover:block absolute right-0 top-0 cursor-pointer z-[2] p-8 bg-foreground/85 rounded-bl-3xl group/link'
						>
							<svg
						...
						<div className='absolute p-8 mr-8 opacity-0 bottom-[150px] group-hover:bottom-0 group-hover:opacity-100 transition-all duration-1000 z-[2]'>
							<h3 className='text-[28px] text-white'>{w.title}</h3>
							<p className='my-4 text-muted-foreground'>{w.desc}</p>
							<Link
								href={w.source}
								className='text-muted-foreground text-[15px] flex gap-2 items-center hover:gap-4 transition-all duration-300 pb-1 border-b-2 border-transparent hover:border-muted-foreground w-fit'
								target='_blank'
							>
								View Code Source
								<svg
...
const WORK = [
	{
		...
		desc: 'Cadency is a social media application developed using React.js, Node.js, SCSS, Express.js, and MySQL as the database.',
